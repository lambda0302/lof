...
class prioritycmp1 {
public:
	bool operator()(my2Dpointers p1, my2Dpointers p2) {
...
//全局变量 用于存储近邻的优先队列
priority_queue<my2Dpointers, vector<my2Dpointers>, prioritycmp1> pq;

//全局变量 用于存储pq存储不完的元素，用于实现k距离内有k+1个点的情况
priority_queue<my2Dpointers, vector<my2Dpointers>, prioritycmp2> pqrecord;

//生成树时用sort对维度x排序
bool cmp1(my2Dpointers pointer1, my2Dpointers pointer2) {
	return pointer1.getpointer1() < pointer2.getpointer1();
}
//生成树时用sort对维度y排序
bool cmp2(my2Dpointers pointer1, my2Dpointers pointer2) {
...
//生成树
myNode* maketree(vector<my2Dpointers> myPointers, int begin, int end, int dimension = 1) {//begin指向第一个，end指向最后一个
	if (begin > end) {
		return nullptr;
	}
...
//计算当前点与目标的距离并赋予结点
void compulatedistance(myNode* Root, my2Dpointers P) {
...
//判断当前点是否应该加入队列
void decideinput(int k, myNode* Root, my2Dpointers P) {
...
//查找k近邻
void findKnn(myNode* Root, my2Dpointers P,unsigned int k, int dimension) {
...
//计算两点间距离
double distance(my2Dpointers p1, my2Dpointers p2) {
...
int main()
{
	...
	*/
																								...
																								//文件输入点集
	cout << "Please enter the count of the pointers :";
	int size = 0;
	cin >> size;
	vector<my2Dpointers> myP(size);
	...
	map<my2Dpointers, double> map1;//用于点与k距离
	map<my2Dpointers, double> map2;//用于点与lrd
	map<my2Dpointers, double> map3;//用于点与lof
